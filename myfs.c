...
void mkfs(char *buffer)
{
	...
	{
		printf("Error: Writing superblock failed=== %s ====\n",strerror(errno));
		return;
	}
	...
	{
		fprintf(stderr, "Error: Writing inode failed\n");
		return;
	}
	close(file_id);
}
...
void cd(char *buffer)
{
	...
	for(int i=0; i<MAX_DATA_BLOCK; i++)
	{
		...
		if(read_status==-1)
		{
			fprintf(stderr,"Error: Unable to read inode_cache\n");
		}
...
void cp(char *buffer)   /* JUST FOR CHECKING PURPOSE ----NOT CORRECT */
{
	...
	if(read_status==-1)
	{
		fprintf(stderr, "Error: Unable to read file sb1\n");
	}
	read_status= read(file_id2,(void *)&sb2, sizeof(sb2));
	if(read_status==-1)
	{
		fprintf(stderr, "Error: Unable to read file sb2\n");
	}
	...
	if(read_status==-1)
	{
		fprintf(stderr, "Error: Unable to read file inode1\n");
	}
	...
	if(read_status==-1)
	{
		fprintf(stderr, "Error: Unable to read file inode2\n");
	}
	for(int j=0; j<num_slash1; j++)
	{
		...
		for(int i=0; i<in1.num_block_count; i++)
		{
			...
			if(read_status==-1)
			{
				fprintf(stderr, "Error: Unable to read file nested_inode\n");
			}
...
void nwfile(char *buffer)
{
	...
	if(read_status==-1)
	{
		fprintf(stderr, "Error: Unable to read from File System\n");
	}
	inode in_cache;
	int i,j;
	for(i=0; i<MAX_INODE; i++)
	...
	int num_dblock= file_size_bytes/sb.block_size;
	lseek(file_id,0,SEEK_SET);
	lseek(file_id,sizeof(sb)+sizeof(inode)*num_in, SEEK_SET);
	in.block_info[in.num_block_count]=i;
	in.num_block_count++;
	...
	FILE *fp;
	fp= fopen(in_cache.file_name,"a+");
	char buff[1000];
	printf("Enter things want to write in file: ");
	gets(buff);
	int len= strlen(buff);
	printf("Length: %d\n",len);
	if(len>0)
		fputs(buff,fp);
	in_cache.size= sizeof(fp);

	/* INODE_CACHE not saved till now not complete */


	write_status= write(file_id, (FILE *)&fp, sizeof(fp));
	if(write_status==-1)
	{
		fprintf(stderr, "Error: Unable to write back into File System\n");
	}
