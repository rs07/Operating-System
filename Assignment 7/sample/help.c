...
void copy_osfile_to_drive(char *filename, char *drive)
{
	int fdsrc,fddest,i=0;
	...
	/*ssize_t write(int fd, const void *buf, size_t count);*/
	int wr_st;
	wr_st = write(fd,(void*)&sn, sizeof(sn));

	read_st = read(fd,(void*)&in, sizeof(in));
	wr_st = write(fd,(void*)&in,sizeof(in));
...
void implement_sys_call(char cmd[][10])
{
	...
	else if(strcmp(cmd[0],CP) == 0)
	{
		...
		{
			copy_file_to_drive(cmd[1],cmd[2]);	
		}
