...
int traverse(int shmid, int semid)
{
    P(semid);
...
int add(int x, int shmid, int semid)
{
...
int del(int x, int shmid, int semid)
{
    ...
    while(1)
    {
    	...
    	switch(ch)
    	{
    		case 1: //P(semid_glob);
    				...
    				scanf("%d",&value);
    				add(value,shmid_glob,semid_glob);
                    //V(semid_glob);
    				break;
    		case 2: printf("Enter the value to be Deleted: \n");
    				scanf("%d",&value);
    				del(value,shmid_glob,semid_glob);
    				break;
    		case 3: traverse(shmid_glob,semid_glob);
    				break;
    		case 4: shmid_cache;
                    key_cache= KEY+(*p);
